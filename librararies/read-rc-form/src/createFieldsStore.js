...
class FieldsStore {
  constructor(fields) {
    this.fields = this.flattenFields(fields);  // 所有表单项与值相关的属性对象的数组集合，如value、name、dirty、touched
    this.fieldsMeta = {}; // 所有表单项与描述表单的对象的数组集合，如name、originalProps、ref、trugger、validate等
  }
  ...
  getFieldValuePropValue(fieldMeta) {
    ...
    return { [valuePropName]: fieldValue };  // valuePropName = 'value'
  }
